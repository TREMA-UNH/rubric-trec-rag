...
source ../new-api-keys.sh

OUT="./data/rag24-together"
mkdir -p $OUT
...
ungraded_gen="rubric-rag24-concat-gen.jsonl.gz"
ungraded_auggen="rubric-rag24-concat-auggen.jsonl.gz"

ungraded="$ungraded_auggen"
#for ungraded in "rubric-rag24-auggen.jsonl.gz" "rubric-rag24-gen.jsonl.gz"; do
for ungraded in "${ungraded_gen}" "${ungraded_auggen}"; do
...
# The final produced file is questions-explain--questions-rate--nuggets-explain--nuggets-rate--all-trecDL2020-qrels-runs-with-text.jsonl.gz
	...
	rsync -Lu $DATA/$ungraded $OUT/$ungraded
...
# grade questions
	...
	withrate="x-questions-rate--${ungraded}"
	withrateextract="x-questions-explain--${withrate}"


	python -O -m exam_pp.exam_grading $OUT/$ungraded -o $OUT/$withrate --prompt-class QuestionSelfRatedPrompt --question-path $DATA/rag24-questions.jsonl.gz  --question-type question-bank  --max-out-tokens 256 --max-tokens=512  --model-pipeline chat-completions --model-name ${model}  --llm-api-key $TOGETHER_API_KEY --llm-base-url https://api.together.xyz/v1 --llm-stop-tokens "<|eot_id|>" "<|eom_id|>" 

	# echo -e "\n\n\ Explained rag24 Questions"
	#python -O -m exam_pp.exam_grading $OUT/$withrate -o $OUT/$withrateextract --prompt-class QuestionSelfRatedUnanswearblePromptWithChoices --question-path $DATA/rag24-questions.jsonl.gz  --question-type question-bank  --max-out-tokens 256 --max-tokens=512  --model-pipeline chat-completions --model-name ${model}  --llm-api-key $TOGETHER_API_KEY --llm-base-url https://api.together.xyz/v1 --llm-stop-tokens "<|eot_id|>" "<|eom_id|>"  --max-queries 10
...
for ungraded in "${ungraded_gen}" "${ungraded_auggen}"; do
	...
	for direct in FagB FagB_few HELM Sun Sun_few Thomas; do
	        ...
	        out="$direct-$in"
uestion-bank 
	        python -O -m exam_pp.exam_grading    $OUT/$in             -o  $OUT/$out                    --prompt-class                $direct          --question-type   direct         --question-path                 $DATA/queries-rag24.json           --max-out-tokens 256 --max-tokens 512  --model-pipeline chat-completions --model-name ${model} --llm-api-key $LLM_API_KEY --llm-base-url https://api.together.xyz/v1 --llm-stop-tokens "<|eot_id|>" "<|eom_id|>" 
...
for promptclass in  QuestionSelfRatedUnanswerablePromptWithChoices ; do  #NuggetSelfRatedPrompt
	...
	final="questions-rate--${ungraded}"
	python -O -m rubric_trec_rag.export_rag_gen_as_run -r $DATA/$ungraded --run-out ./data/${ungraded_base}-runs

	for minrating in 1 2 3 4 5; do
		echo ""
		#python -O -m exam_pp.exam_evaluation $DATA/$final --question-set question-bank --prompt-class $promptclass --min-self-rating $minrating --leaderboard-out $DATA/rag24-autograde-cover-leaderboard-$promptclass-minrating-$minrating.solo.${ungraded_base}.tsv 

		# N.B. requires TREC-rag24 runs to be populated in $DATA/ragruns
		# python -O -m exam_pp.exam_evaluation $DATA/$final --question-set question-bank --prompt-class $promptclass -q $DATA/rag24-autograde-qrels-leaderboard-$promptclass-minrating-$minrating.solo.${ungraded_base}.qrels  --min-self-rating $minrating --qrel-leaderboard-out $DATA/rag24-autograde-qrels-$promptclass-minrating-$minrating.solo.${ungraded_base}.tsv --run-dir $DATA/${ungraded_base}-runs 
        
		# Since generative IR systems will not share any passages, we represent them as special run files
		python -O -m exam_pp.exam_evaluation $DATA/$final --question-set question-bank --prompt-class $promptclass -q $DATA/rag24-autograde-qrels-leaderboard-$promptclass-minrating-$minrating.solo.${ungraded_base}.qrels  --min-self-rating $minrating --qrel-leaderboard-out $DATA/rag24-autograde-qrels-$promptclass-P20-minrating-$minrating.solo.${ungraded_base}.gen.tsv --run-dir $DATA/${ungraded_base}-runs 
	done
	python -O -m exam_pp.exam_evaluation $DATA/$final --question-set question-bank --prompt-class $promptclass --trec-eval-metric "ndcg_cut.20" -q $DATA/rag24-autograde-qrels-leaderboard-$promptclass-minrating-$minrating.solo.${ungraded_base}.qrels  --min-self-rating $minrating --qrel-leaderboard-out $DATA/rag24-autograde-qrels-$promptclass-NDCG20.solo.${ungraded_base}.gen.tsv --run-dir $DATA/${ungraded_base}-runs 
    done
done
